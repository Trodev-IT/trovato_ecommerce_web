...
<head>
    ...
    <title>Pending Orders | Ashar Alo Shop</title>
        ...
        <div class="layout-page">
            ...
            <div class="content-wrapper">
                ...
                <div class="container-xxl flex-grow-1 container-p-y">
                    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Orders/</span> Pending Orders</h4>
                    ...
                    <div class="row">
                        ...
                        <div class="search-container mb-3">
                            <input type="text" id="transaction-search" class="form-control" placeholder="Search Transaction ID...">
                        </div>
                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <div>
                                    <a href="{{route('ontheways',['id'=>$id->id])}}" class="btn btn-secondary" style="color: white">On The Way</a>
                                </div>
                            </div>
                            <div class="table-responsive text-nowrap">
                                <table id="order-table" class="table table-hover">
                                    ...
                                    @foreach($orders as $cart)
                                        <form action="{{route('insertDate')}}" method="post">
                                            @csrf

                                            <input type="hidden" name="user" value="{{$id->id}}" />
                                            <input type="hidden" name="id" value="{{$cart->id}}">
                                        <tr>
                                            <td><strong>#{{$cart->order_no}}</strong></td>
                                            <input type="hidden" name="orderids" value="{{$cart->order_no}}" />
                                            <td>{{$cart->product_name}}</td>
                                            <input type="hidden" name="proid" value="{{$cart->product_id}}" />
                                            <td>{{$cart->quantity}}</td>
                                            <input type="hidden" name="quantity" value="{{$cart->quantity}}" />
                                            <td>{{$cart->transaction_id}}</td>
                                            <input type="hidden" name="transaction_id" class="form-control" value="{{$cart->transaction_id}}" />
                                            <td>{{$cart->payment}}</td>
                                            <td>
                                                <input type="date" name="date" class="form-control" value="{{$cart->delivery_date}}" />
                                            </td>
                                            <td>{{$cart->shipping_address}}</td>
                                            <td>{{$cart->city}}</td>
                                            <td>{{$cart->phone}}</td>
                                            <td>
                                                <button type="submit" class="btn btn-outline-primary">{{$cart->status}}</button>
                                                </td>
                                        </tr>
                                        </form>
                                    @endforeach
                        ...
                        <script>
                            ...
                            document.getElementById('transaction-search').addEventListener('input', function() {
                                ...
                                rows.forEach(row => {
                                    const transactionId = row.querySelector('td:nth-child(4)').textContent.toLowerCase();
